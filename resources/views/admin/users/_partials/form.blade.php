...
<div class="">
    ...
    <input class="w-full px-5 py-2 text-gray-700 bg-gray-200 rounded" id="name" name="name" type="text" placeholder="Nome" aria-label="Name" name="name" value="{{ $user->name ?? old('name') }}">
</div>
<div class="mt-2">
    <label class="block text-sm text-gray-600" for="email">E-mail</label>
    <input class="w-full px-5  py-2 text-gray-700 bg-gray-200 rounded" id="email" type="email" placeholder="E-mail" aria-label="Email" name="email" value="{{ $user->email ?? old('email') }}">
</div>
<div class="mt-2">
